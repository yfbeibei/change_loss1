...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # print(f"out_point shape: {out_point.shape}")
        # print(f"tgt_point shape: {tgt_point.shape}")
        # print(f"tgt_ids shape: {tgt_ids.shape}")

        # # 确保 tgt_ids 包含有效的索引
        # if tgt_ids.max() >= self.num_classes:
        #     raise ValueError(f"tgt_ids 中的无效类索引: {tgt_ids.max()} >= {self.num_classes}")
