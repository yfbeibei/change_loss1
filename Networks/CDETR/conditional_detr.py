...
class ConditionalDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, channel_point, aux_loss=False, dm_decoder=None, branch_merge=False, branch_merge_way=1, transformer_flag=""):
        ...
        # 替换为可变形卷积
        # self.input_proj = DeformConv2d(backbone.num_channels, hidden_dim, kernel_size=3, padding=1)
        # Offset计算层
        # self.offset_conv = nn.Conv2d(backbone.num_channels, 18, kernel_size=3, padding=1)
        self.density_proj = nn.Conv2d(32, 256, kernel_size=1)
    ...
    def forward(self, samples: NestedTensor):
        ...
        # print("Number of features:", len(features))  # 打印 features 列表的长度
        shallow_feature = features[0].decompose()[0]  # 浅层特征图
        medium_feature = features[1].decompose()[0]  # 中层特征图
        src, mask = features[-1].decompose()
        # # 计算 offset
        # offset = self.offset_conv(src)
        if self.dm_decoder is not None:
            out_dm=self.dm_decoder(shallow_feature,medium_feature,src)
            ...
            if self.branch_merge:
                ...
                if self.branch_merge_way==2:
                    ...
                    src=self.input_proj(src)+out_density  #特征图的投影和上采样后的密度图相加，将密度图信息融合到特征图中16X16
        else:
            out_dm=None
...
class dm_decoder2(nn.Module):
    def __init__(self,dim_feedforward=2048, hidden_dim=256, hidden_dim2=128,hidden_dim3=512):
    ...
    def forward(self,shallow_feature,medium_feature,x):
        ...
        shallow_feature = F.upsample_bilinear(shallow_feature, scale_factor=2)#[8,256,128,128]
        shallow=self.reglayer1(shallow_feature) #[8,128,128,128]
        x_shallow=shallow.clone()
        mu_shallow = self.density_layer(x_shallow) #[8,1,128,128]
        mu2_shallow = F.relu(mu_shallow) #[8,1,128,128]
        B, C, H, W=mu2_shallow.size() 
        mu2_sum_shallow = mu2_shallow.view([B,-1]).sum(1).unsqueeze(1).unsqueeze(2).unsqueeze(3) #[8,1,1,1]
        mu2_normed_shallow = mu2_shallow / (mu2_sum_shallow + 1e-6) #[8,1,128,128]

        medium_feature = F.upsample_bilinear(medium_feature, scale_factor=2)#[8,512,64,64]
        medium=self.reglayer2(medium_feature) #[8,128,64,64]
        x_medium=medium.clone()
        mu_medium = self.density_layer(x_medium) #[8,1,64,64]
        mu2_medium = F.relu(mu_medium) #[8,1,64,64]
        B, C, H, W=mu2_medium.size() 
        mu2_sum_medium = mu2_medium.view([B,-1]).sum(1).unsqueeze(1).unsqueeze(2).unsqueeze(3) #[8,1,1,1]
        mu2_normed_medium = mu2_medium / (mu2_sum_medium + 1e-6) #[8,1,64,64]

        return [self.reg_layer2(x2), mu2, mu2_normed,mu2_shallow,mu2_normed_shallow,mu2_medium,mu2_normed_medium]
